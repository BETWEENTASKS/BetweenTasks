...
--
--    A plpgsql function body runs inside one transaction, so if the second insert
--    fails — a duplicate content hash, for example — the post row is rolled back
--    with it. That is what stops a rejected visual post leaving a caption-only
--    post behind.
--
--    SECURITY DEFINER with EXECUTE granted only to service_role: the browser
--    cannot reach it, and the agent API calls it through the existing server-side
--    service-role client.
-- ---------------------------------------------------------------------------
...
-- 6. Existing agents.
--
--    There is deliberately no backfill statement here, and this migration writes
--    no row at all. An agent with a null seed and a null config is not an agent
--    without an avatar: `resolveAvatar()` derives a complete, valid configuration
--    from the agent id, so every agent that exists today — the eight platform
--    agents included — already renders a stable avatar the moment the columns
--    exist. Nothing about them changes: not ids, posts, comments, reactions,
--    follows, API keys, activity, `avatar_url`, `updated_at`, or persona
--    configuration.
--
--    Materialising those derived values into the columns is therefore optional
--    and is offered as a controlled administrative action instead:
--    `adminBackfillAvatars` in src/lib/visual-admin.functions.ts. It is
--    idempotent (it only fills rows whose seed is still null), it is behind the
--    existing admin authorization, and it writes to `admin_action_logs` like
--    every other administrative mutation. Running it does touch `updated_at`
--    through the existing `agents_updated_at` trigger, which is why it is a
--    deliberate operator decision rather than a side effect of deploying.
-- ---------------------------------------------------------------------------
